...
from core.model.layers.phydnet_constrain_moments import K2M
from core.model.layers.phydnet_phycells import BasePhyCell_Cell, PhyCell_Cell_1, PhyCell_Cell_AdvDiff
...
class PredRNN_Cell(BaseModel):
    def __init__(self, input_shape, input_dim, hidden_dim, kernel_size):
        ...
        self.conv_m = nn.Conv2d(in_channels=self.hidden_dim,
                              out_channels=3 * self.hidden_dim,
                              kernel_size=self.kernel_size,
                              padding=self.padding, bias=False)
        
        self.conv_o = nn.Conv2d(in_channels=2 * self.hidden_dim,
                              out_channels=self.hidden_dim,
                              kernel_size=self.kernel_size,
                              padding=self.padding, bias=False)
        
        self.conv_last = nn.Conv2d(in_channels=2 * self.hidden_dim,
                              out_channels=self.hidden_dim,
                              kernel_size=1, padding=0, bias=False)

    def forward(self, x, hidden):  # x [batch, hidden_dim, width, height]
        h_cur, c_cur, m_curr = hidden

        x_conv = self.conv_x(x)
        h_conv = self.conv_h(h_cur)
        m_conv = self.conv_m(m_curr)
        i_x, f_x, o_x, g_x, i_x_new, f_x_new, g_x_new = torch.split(x_conv, self.hidden_dim, dim=1)
        ...
        i_new = torch.sigmoid(i_x_new + i_m)
        f_new = torch.sigmoid(f_x_new + f_m)
        g_new = torch.tanh(g_x_new + g_m)
        
        m_next = f_new * m_curr + i_new * g_new

        c_m_cat = torch.cat((c_next, m_next), dim=1)
        o = torch.sigmoid(o_x + o_h + self.conv_o(c_m_cat))
        
        h_next = o * torch.tanh(self.conv_last(c_m_cat))
...
class PredRNN(BaseModel):
    def __init__(self, input_shape, input_dim, hidden_dims, n_layers, kernel_size):
        ...
        for i in range(0, self.n_layers):
            ...
            print('PredRNN layer ', i, 'input dim ', cur_input_dim, ' hidden dim ', self.hidden_dims[i])
            cell_list.append(PredRNN_Cell(input_shape=self.input_shape,
    ...
    def forward(self, input_, first_timestep=False):  # input_ [batch_size, 1, channels, width, height]
        ...
        for j, cell in enumerate(self.cell_list):
            if j == 0:  # bottom layer
                self.H[j], self.C[j], self.M = cell(input_, (self.H[j], self.C[j], self.M))
            else:
                self.H[j], self.C[j], self.M = cell(self.H[j - 1], (self.H[j], self.C[j], self.M))
                
        return (self.H, self.C, self.M), self.H  # (hidden, output)
    ...
    def initHidden(self, batch_size):
        ...
        self.M = torch.zeros((batch_size, self.hidden_dims[0], self.input_shape[0], self.input_shape[1]),
                                      device=torch.device(self.device), dtype=self.dtype)
